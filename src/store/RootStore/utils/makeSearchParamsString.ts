...
export const makeSearchParamsString = <K extends keyof TParams>({
  ...
  const baseParamsSearchString = stringifySearchParams(baseParams);
  ...
  if (extraParams) {
    for (const [key, value] of Object.entries(extraParams) as [K, TParams[K]][]) {
      if (
        ...
        extraParamsSearchStringArray.push(stringifySearchParams(QUERY_PARAMS_CONVERT_RULES[key](value as TParams[K])));
      }
    }
  }
